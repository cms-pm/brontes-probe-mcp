...
class SessionManager:
    ...
    def _write_meta(self, data: dict[str, Any], profile: str) -> None:
        self._meta_path(profile).write_text(json.dumps(data))

    def _read_meta(self, profile: str) -> dict[str, Any] | None:
        try:
    ...
    def _remove_meta(self, profile: str) -> None:
        try:
    ...
    def _active_profiles(self) -> list[str]:
        return [
            p.stem for p in self._log_dir().glob("*.json") if not p.name.startswith(".")
        ]
    ...
    def stop(self, force: bool = False) -> SessionStatus:
        with _OperationLock(self._lock_path()):
            for profile_name in self._active_profiles():
                meta = self._read_meta(profile_name)
                if meta is None:
                    continue
                pid_raw = meta.get("pid")
                if pid_raw is not None:
                    ...
                    try:
                        ...
                        pass
                self._remove_meta(profile_name)
